...
import { Cinema, Movie, ResponseList, ResponseOne, ResponseWithError, Show } from '../interfaces/interfaces';
...
export class ShowtimesByCinemaService {
  ...
  getShowtime(id: number,moreDetails :string | null = null): Observable<any>{
    if(moreDetails != null && moreDetails?.toLocaleLowerCase() == "yes"){
      return this.http.get<ResponseOne<Show> | ResponseWithError>(`${this.showtimeUrl}/${id}?moredetails=${moreDetails}`);
    }
      return this.http.get<ResponseOne<Show> | ResponseWithError>(`${this.showtimeUrl}/${id}`);
  }

  getMovieData(id :number): Observable<any>{
    return this.http.get<ResponseOne<Movie> | ResponseWithError>(`${this.movieUrl}/${id}`);
  }
